...
void ParticleRenderer::_drawPoints()
{
    if (!m_vbo)
    {
        glBegin(GL_POINTS);  //delimit the vertices of a primitive or a group of like primitives
        {
            ...
            {
                glVertex3fv(&m_pos[k]);  //glVertex commands are used within glBegin/glEnd pairs to specify point,
                                         //line, and polygon vertices. The current color, normal, texture coordinates,
                                         //and fog coordinate are associated with the vertex when glVertex is called.
                k += 4;
            }
        }
        glEnd();
